class grandmahouse_lv1 extends Phaser.Scene {
  ...
  create() {
    ...
    this.playerPositions = []; // Clear path history
    ...
    // Reset hearts to 3 and clear inventory
    ...
    // Delay for UIScene time to boot up before sending commands
    this.time.delayedCall(150, () => {
        let ui = this.scene.get("UIScene");
        if (ui) {
            ui.setInventoryMode(1);
            if (ui.clearInventory) ui.clearInventory(); // Clear old icons
            ui.updateHearts(this.gameManager.hearts);   // 3 hearts
    ...
    map.createLayer("H_wall", tilesArray, 0, 0);
    map.createLayer("H_landwallprop", tilesArray, 0, 0);
    map.createLayer("H_furniture", tilesArray, 0, 0);
    map.createLayer("H_prop2", tilesArray, 0, 0);
